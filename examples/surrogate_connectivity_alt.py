...
from mne_connectivity.datasets.surrogate import make_surrogate_data_alt
...
fmin, fmax = 3, 23
pretrial_data = epochs.copy().crop(tmin=None, tmax=-0.5)
...
pretrial_con = spectral_connectivity_epochs(
    pretrial_data, method="imcoh", indices=indices, fmin=fmin, fmax=fmax
)
...
pretrial_surrogates = make_surrogate_data_alt(
    pretrial_data, n_shuffles=n_shuffles, rng_seed=44
)
...
freqs = np.array(pretrial_con.freqs)
...
poststim_data = epochs.copy().crop(tmin=0, tmax=None)

# Compute connectivity for post-stimulus data
poststim_con = spectral_connectivity_epochs(
    poststim_data, method="imcoh", indices=indices, fmin=fmin, fmax=fmax
)
...
poststim_surrogates = make_surrogate_data_alt(
    poststim_data, n_shuffles=n_shuffles, rng_seed=44
)
