...
def make_surrogate_data(data, n_shuffles=1000, rng_seed=None, return_generator=True):
    ...
    for each channel. This destroys the covariance of the data, such that connectvity
    estimates should reflect the null hypothesis of no connectivity between signals
    (i.e. only background noise) :footcite:`PellegriniEtAl2023`.

    For the surrogate data to properly reflect a null hypothesis, the data which is
    shuffled **must not** have a temporal structure which is consistent across epochs.
    This is because even after shuffling the epochs, there will still be a high degree
    of residual connectivity between channels. As a result, connectivity estimates from
    your surrogate data will capture connectivity information of interest instead of the
    ...
    significant connectivity in your data. Examples of data with a consistent temporal
    structure include evoked potentials, where a stimulus is presented or an action
    performed at a set time during each epoch.
    ...
    a noise bias for one set of estimates (fewer epochs gives noisier connectivity
    estimates).
