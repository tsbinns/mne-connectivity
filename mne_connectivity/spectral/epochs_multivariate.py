            ...
            while not create_filter and not stop:
                for time_i in range(n_times):
                    for freq_i in range(self.n_freqs):
                        if np.all(alpha[time_i, freq_i] == beta[time_i, freq_i]):
                            create_filter = True
                            break
                stop = True
            if create_filter:
                n_chans = E.shape[2]
                eye_4d = np.zeros_like(alpha)
                eye_4d[:, :, np.arange(n_chans), np.arange(n_chans)] = 1
                alpha = eye_4d
                beta = np.rot90(eye_4d, axes=(2, 3))
...
class _CaCohEst(_MultivariateCohEstBase):
    ...
    def _compute_con_daughter(
        ...
        # allocate space for fitted filters
        # (used for CSD deflation if multiple components are being fit)
        if n_components > 1:  # don't need if only 1 component being fit
            W_a = np.zeros((*U_bar_aa.shape[:2], U_bar_aa.shape[3], n_components))
            W_b = np.zeros((*U_bar_bb.shape[:2], U_bar_bb.shape[3], n_components))

        # get starting basis of space for seeds and targets
        # (used for CSD deflation if multiple components are being fit)
        B_a = np.broadcast_to(
            np.identity(U_bar_aa.shape[3]), (*W_a.shape[:3], W_a.shape[2])
        )
        B_b = np.broadcast_to(
            np.identity(U_bar_bb.shape[3]), (*W_b.shape[:3], W_b.shape[2])
        )
        ...
        for comp_i in range(n_components):
            ...
            if comp_i + 1 < n_components:  # don't do on last component
                ...
                W_a[..., comp_i] = np.squeeze(alpha, axis=-1)
                W_b[..., comp_i] = np.squeeze(beta, axis=-1)

                # deflate CSD to fit next component
                C_bar, B_a, B_b = self._deflate_csd(
                    C_bar,
                    W_a[..., : comp_i + 1],
                    W_b[..., : comp_i + 1],
                    B_a,
    ...
    def _compute_filters_patterns(
        ...
        # Project filters back to pre-deflation space
        alpha = B_a @ alpha
        beta = B_b @ beta
    ...
    def _deflate_csd(self, C, W_a, W_b, B_a, B_b, n_seeds):
        ...
        NeuroImage, DOI: 10.1016/j.neuroimage.2014.03.075, for more information.
        """
        # get orthogonal basis space for filters
        # (streamlined version of scipy.linalg.null_space() suited for our purposes)
        B_a = np.linalg.svd(W_a)[0][..., 1:]
        B_b = np.linalg.svd(W_b)[0][..., 1:]
    ...
    def _null_space(self, A):  # XXX: REMOVE LATER
        """Test."""
        V, S, _ = np.linalg.svd(A)
        tol = np.amax(S) * 1e-6
        num = np.sum(S > tol, axis=2, dtype=int).min()  # should all be the same
